...
static void test_semantic_mutable_block_fast_path_matches_steps(void) {
    ...
    TEST_CHECK(cpu.er[0] == stepped.er[0]);
    TEST_CHECK(cpu.semantic_fast_blocks == 1);
    TEST_CHECK(cpu.semantic_fast_cycles == 2);
    TEST_CHECK(cpu.semantic_mutable_fast_blocks == 1);
    TEST_CHECK(cpu.semantic_mutable_fast_cycles == 2);
    TEST_CHECK(cpu.semantic_fast_rejects == 0);
    teardown();
}
...
static void test_hot_prefix0100_plain_memory_step_fast_path(void) {
    setup();
    memory_init(&bus.external_ram, bus.machine->ram_size, true);
    bus_build_memory_map(&bus);

    uint32_t base = bus.machine->ram_base + 0x340;
    uint32_t address = base + 8;
    bus_write32(&bus, address, 0x89abcdefu);
    write_code16(0, 0x0100);
    write_code16(2, 0x6F10); /* MOV.L @(d:16,ER1),ER0 */
    write_code16(4, 0x0008);
    cpu.pc = CODE_BASE;
    cpu.ccr = CCR_I;
    cpu.er[1] = base;

    h8s_cpu_step(&cpu);
    TEST_CHECK(cpu.pc == CODE_BASE + 6);
    TEST_CHECK(cpu.er[0] == 0x89abcdefu);
    TEST_CHECK(cpu.hot_prefix0100_plain_memory_instructions == 1);
    TEST_CHECK(cpu.cycle_count == 1);
    TEST_CHECK(cpu.ccr & CCR_N);
...
static void test_semantic_reject_cache_counts_state_independent_hits(void) {
    ...
    TEST_CHECK(cpu.semantic_reject_backoff == H8S_SEMANTIC_CACHED_REJECT_BACKOFF);
    teardown();
}
