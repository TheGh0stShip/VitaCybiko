...
static void test_plain_memory_instruction_reads_absolute32_long_ram(void)
{
    const uint8_t code[] = {
        0x01, 0x00, 0x6b, 0x23, 0x00, 0x40, 0x01, 0x20
    }; /* MOV.L @0x00400120:32,ER3 */
    uint8_t rom[8] = {0};
    memcpy(rom, code, sizeof(code));
    h8s_block_t block;
    TEST_ASSERT(h8s_analyze_rom_block(rom, sizeof(rom), 0, 4, &block));
    TEST_ASSERT(block.instructions == 1);
    TEST_CHECK(block.decoded[0].bytes == 8);
    TEST_CHECK(block.decoded[0].imm == 0x6b230040);
    TEST_CHECK(block.decoded[0].ext == 0x0120);

    uint32_t er[8] = {0};
    address_bus_t bus;
    h8s_cpu_t cpu;
    setup_memory_equiv_cpu(&bus, &cpu, code, sizeof(code), er, 0);
    memory_write32(&bus.external_ram, 0x120, 0x89abcdef);
    ...
    TEST_CHECK(state.er[3] == 0x89abcdef);
    TEST_CHECK(state.ccr == cpu.ccr);
    TEST_CHECK(state.pc == sizeof(code));
    teardown_memory_equiv_cpu(&bus);
}
...
static void test_mixed_plain_block_exit_executes_bsr16(void)
{
    const uint8_t code[] = {
        0x0b, 0x00,       /* 0: ADDS #1,ER0 */
        0x5c, 0x00,       /* 2: BSR d:16 */
        0x00, 0x06,       /* target = 12 */
        0x0b, 0x01,       /* 6: fall-through */
        0x54, 0x70,       /* 8: RTS */
        0x0b, 0x02,       /* 10: skipped padding */
        0x0b, 0x03        /* 12: target */
    };
    h8s_block_t block;
    TEST_ASSERT(h8s_analyze_rom_block(code, sizeof(code), 0, 8, &block));
    TEST_ASSERT(block.instructions == 1);
    TEST_CHECK(block.branch_kind == H8S_BLOCK_BRANCH_BSR16);
    TEST_CHECK(block.branch_target == 12);
    TEST_CHECK(h8s_mixed_plain_block_supported(&block));
    ...
    h8s_branch_edge_cache_init(&edge_cache);
    h8s_block_cpu_state_t state = {.ccr = 0, .pc = 0};
    state.er[7] = bus.machine->ram_base + 0x120;
    uint32_t next = 0;
    ...
    TEST_CHECK(h8s_execute_mixed_plain_block_exit(&block, &edge_cache, &bus,
                                                  ...
                                                  &state, &next));
    TEST_CHECK(state.er[0] == 1);
    TEST_CHECK(state.er[7] == bus.machine->ram_base + 0x11c);
    TEST_CHECK(bus_read32(&bus, state.er[7]) ==
               ((bus.machine->ram_base + 6u) & 0xffffffu));
    TEST_CHECK(next == 12);
    TEST_CHECK(edge_cache.misses == 1);
...
static void test_mixed_plain_block_exit_executes_rts(void)
{
    const uint8_t code[] = {
        0x0b, 0x00, /* ADDS #1,ER0 */
        0x54, 0x70  /* RTS */
    ...
    TEST_CHECK(block.branch_op == 0x5470);
    TEST_CHECK(h8s_mixed_plain_block_supported(&block));
    ...
    h8s_block_cpu_state_t state = {.ccr = 0, .pc = 0};
    state.er[7] = bus.machine->ram_base + 0x100;
    bus_write32(&bus, state.er[7], 0xa5123456);
    uint32_t next = 0;
    ...
    TEST_CHECK(h8s_execute_mixed_plain_block_exit(&block, NULL, &bus,
                                                  ...
                                                  &state, &next));
    TEST_CHECK(state.er[0] == 1);
    TEST_CHECK(state.er[7] == bus.machine->ram_base + 0x104);
    TEST_CHECK(next == 0x123456);
    TEST_CHECK(state.ccr == 0);
    teardown_memory_equiv_cpu(&bus);
}
