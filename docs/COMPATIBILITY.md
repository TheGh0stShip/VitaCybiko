...
| --- | --- |
| Classic V2 / CyOS 1.3.58 | Genuine first-run setup, Main Desktop, Pinball Pro gameplay and Calculator 2 + 3 = 5 in Windows Vita3K |
...
| Persistence | Setup survives Vita3K restart; clock sidecar round-trip and elapsed-time logic tested on host; guest clock accuracy still requires validation |
...
The guest desktop clock has shown discrepancies from the persisted RTC registers;
do not rely on it for alarms or timekeeping. This remains an open correctness issue.
...
Esc did not reliably leave the tested apps; use Start + Select to save and
return to the model menu. Guest exit behavior remains under investigation.
